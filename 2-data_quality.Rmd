...
read_stats <- fromJSON("data/reports/reads_multiqc_data.json")$report_saved_raw_data$multiqc_general_stats %>% 
  map_dfr(as_tibble, .id = "microsample") %>% 
  rename(total_sequences=FastQC_mqc_generalstats_fastqc_total_sequences,
         total_duplicated_percentage=FastQC_mqc_generalstats_fastqc_percent_duplicates,
         percent_gc=FastQC_mqc_generalstats_fastqc_percent_gc) %>% 
  select(microsample,total_sequences,percent_gc,total_duplicated_percentage) %>% 
  mutate(microsample = str_extract(microsample, "M\\d+")) %>%
  group_by(microsample) %>%
  summarise(total_sequences=sum(total_sequences)*1000000, percent_unique=100-mean(total_duplicated_percentage), percent_gc=mean(percent_gc))
```
...
quantification_stats  <- read_tsv("data/reports/quantify_samtools-flagstat-dp_Percentage_of_total.txt") %>%
   filter(str_detect(Sample, "mgg-pbdrep")) %>%
   mutate(Sample = str_extract(Sample, "M\\d+")) %>%
    rename(microsample=Sample,reads_mapped_percent=`Properly Paired`) %>%
    group_by(microsample) %>%
    summarise(reads_mapped_percent=mean(reads_mapped_percent))
```
...
quality_stats %>%
   ...
   mutate(animal=substr(cryosection,1,4)) %>%
    ggplot(aes(x=total_sequences,y=microsample,fill=log(size)))+
      geom_col()+
...
quality_stats %>%
    left_join(sample_metadata,by=join_by(microsample==microsample)) %>%
    mutate(animal=substr(cryosection,1,4)) %>%
    ggplot(aes(x=percent_unique,y=microsample,fill=log(size)))+
      geom_col()+
...
quality_stats %>%
    ...
    mutate(animal=substr(cryosection,1,4)) %>%
    ggplot(aes(x=percent_gc,y=microsample,fill=log(size)))+
      geom_col()+
...
quality_stats %>%
    ...
    filter(section != "Ileum") %>%
    mutate(animal=substr(cryosection,1,4)) %>%
    ggplot(aes(x=reads_mapped_host_percent,y=microsample,fill=log(size)))+
      geom_col()+
      facet_nested(batch + section + type ~ ., scales="free", space="free", switch = "y") +
      theme(strip.text.y.left = element_text(angle = 0)) +
...
quality_stats %>%
    left_join(sample_metadata,by=join_by(microsample==microsample)) %>%
    mutate(animal=substr(cryosection,1,4)) %>%
    ggplot(aes(x=reads_mapped_human_percent,y=microsample,fill=log(size)))+
      geom_col()+
...
quality_stats %>%
    ...
    mutate(animal=substr(cryosection,1,4)) %>%
    ggplot(aes(x=reads_mapped_percent,y=microsample,fill=log(size)))+
      geom_col()+
...
quality_stats %>%
    ...
    mutate(animal=substr(cryosection,1,4)) %>%
    ggplot(aes(x=percent_gc,y=total_sequences,color=log(size)))+
      geom_point()+
...
quality_stats %>%
    ...
    filter(type == "Positive") %>%
    mutate(animal=substr(cryosection,1,4)) %>%
    ggplot(aes(x=percent_gc,y=percent_unique,color=log(size)))+
      geom_point()+
      facet_nested(. ~ batch, scales="free")+
...
quality %>%
    ...
    filter(section != "Ileum") %>%
    mutate(animal=substr(cryosection,1,4)) %>%
    ggplot(aes(x=quality,y=microsample,fill=log(size)))+
      geom_col()+
      geom_vline(xintercept=5, linetype="dashed", color = "red", size=1) + 
      facet_nested(batch + section + type ~ ., scales="free", space="free", switch = "y") +
      theme(strip.text.y.left = element_text(angle = 0)) +
